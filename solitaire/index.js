...
// include: /var/folders/2x/b7sbnjnd1xz7w5lm_bw59qw00000gn/T/tmpffdue_yd.js
...
var ASM_CONSTS = {
  240552: () => { if (document.fullscreenElement) return 1; },  
 240598: () => { return document.getElementById('canvas').width; },  
 240650: () => { return parseInt(document.getElementById('canvas').style.width); },  
 240718: () => { document.exitFullscreen(); },  
 240745: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 240818: () => { if (document.fullscreenElement) return 1; },  
 240864: () => { return document.getElementById('canvas').width; },  
 240916: () => { return screen.width; },  
 240941: () => { document.exitFullscreen(); },  
 240968: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 241101: () => { return window.innerWidth; },  
 241127: () => { return window.innerHeight; },  
 241154: () => { if (document.fullscreenElement) return 1; },  
 241200: () => { return document.getElementById('canvas').width; },  
 241252: () => { return parseInt(document.getElementById('canvas').style.width); },  
 241320: () => { if (document.fullscreenElement) return 1; },  
 241366: () => { return document.getElementById('canvas').width; },  
 241418: () => { return screen.width; },  
 241443: () => { return window.innerWidth; },  
 241469: () => { return window.innerHeight; },  
 241496: () => { if (document.fullscreenElement) return 1; },  
 241542: () => { return document.getElementById('canvas').width; },  
 241594: () => { return screen.width; },  
 241619: () => { document.exitFullscreen(); },  
 241646: () => { if (document.fullscreenElement) return 1; },  
 241692: () => { return document.getElementById('canvas').width; },  
 241744: () => { return parseInt(document.getElementById('canvas').style.width); },  
 241812: () => { document.exitFullscreen(); },  
 241839: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 241897: () => { return screen.width; },  
 241922: () => { return screen.height; },  
 241948: () => { return window.screenX; },  
 241975: () => { return window.screenY; },  
 242002: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 242055: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 242126: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 242183: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 242439: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 242510: () => { if (document.fullscreenElement) return 1; },  
 242556: () => { return window.innerWidth; },  
 242582: () => { return window.innerHeight; },  
 242609: () => { if (document.pointerLockElement) return 1; },  
 242656: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 244814: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 244978: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 245082: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 248081: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 248147: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 248540: ($0) => { miniaudio.untrack_device_by_index($0); },  
 248583: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 248708: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
