...
float getwaves(vec2 position, int iterations){
	float iter = 0.0;
    float phase = 3.0;    // default 6.0
    float speed = 0.3;    // default 2.0
    float weight = 1.0;
    float w = 0.0;
    float ws = 0.0;
...
void main() {
    vec3 color = mainImage(fragPos * iResolution.xy).xyz;
    fragColor = vec4(hueRotate(color, 5.6), 1.0);
}
